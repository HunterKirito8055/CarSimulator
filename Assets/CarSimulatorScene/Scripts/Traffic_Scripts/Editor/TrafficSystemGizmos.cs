            ...
            foreach (Segment segment in script.segments) {
                ...
                for (int j = 0; j < segment.waypoints.Count; j++) {
                    ...
                    if (pNext != Vector3.zero) {
                        if (segment == script.curSegment) {
                            Gizmos.color = new Color(0f, 0f, 1f); //new Color(1f, .3f, .1f);
                        } else {
                ...
                foreach (Segment nextSegment in segment.nextSegments) {
                    if (nextSegment != null) {
                       ...
                       // Vector3 p1 = segment.waypoints[segment.waypoints.Count - segment.segmentlastWaypoint[segment.nextSegments.IndexOf(nextSegment)] - 1].GetVisualPos();
                        Vector3 p2 = nextSegment.waypoints[segment.waypointNextSegment[segment.nextSegments.IndexOf(nextSegment)]].GetVisualPos(); //Code by AShish
