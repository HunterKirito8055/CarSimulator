...
public class RCC_Camera : MonoBehaviour
{
    ...
    public enum CameraMode { TPS, FPS, WHEEL, FIXED, CINEMATIC, TOP, BACK }
    ...
    void Update()
    {
        ...
        playerSpeed = Mathf.Lerp(playerSpeed, playerCar.speed, Time.unscaledDeltaTime * 15f); // 31-08-2021 //Code Changed By Ashish 
        ...
        thisCam.fieldOfView = Mathf.Lerp(thisCam.fieldOfView, targetFieldOfView, Time.unscaledDeltaTime * 5f);//Code Changed By Ashish
    ...
    private void TPS()
    {
        // if(GameManager.instance.levelTargetSystem.GetVehicleTypeChapter() == 1) //if selected level is bus
        if (GameMaster.instance.selected_vehicle == "Bus")
        {
            TPSDistance = 9.55f;
            TPSHeight = 2.92f;
        }
        ...
        {
            TPSDistance = distance;
            TPSHeight = height;
        }
        //Ashish Code Changed
        if (lastDirection != playerCar.direction)
        {
            if (cameraMode == RCC_Camera.CameraMode.BACK)
            {
                direction = -1;
            }
            else
                direction = 1;// playerCar.direction; 
            orbitX = 0f;
            orbitY = 0f;
        }
        ...
        // Damp the height.
        currentHeight = Mathf.Lerp(currentHeight, wantedHeight, TPSHeightDamping * Time.fixedUnscaledDeltaTime); //Code Changed By Ashish

        // Damp the rotation around the y-axis.
        if (Time.time > 1)
            currentRotation = Quaternion.Lerp(currentRotation, wantedRotation, 1f - Mathf.Exp(-TPSRotationDamping * Time.unscaledDeltaTime)); //Code Changed By Ashish
        ...
        pivot.transform.localPosition = Vector3.Lerp(pivot.transform.localPosition, collisionPos, Time.unscaledDeltaTime * 10f);//Code Changed By Ashish
        pivot.transform.localRotation = Quaternion.Lerp(pivot.transform.localRotation, collisionRot, Time.unscaledDeltaTime * 10f);//Code Changed By Ashish
