...
public class RCC_UIDashboardDisplay : MonoBehaviour {
	...
	public GameObject LeftControllerButtons;
	public GameObject RightControllerButtons;
	public GameObject controllerButtons; //controllers settings
	...
	void Update(){
		...
		case DisplayType.Full:
			...
			if(controllerButtons && !controllerButtons.activeInHierarchy)
				controllerButtons.SetActive(true); //true
		...
		if (RCC_SceneManager.Instance.activePlayerVehicle) {
			...
			if (KMHLabel) {
				...
				if (RCCSettings.units == RCC_Settings.Units.KMH)
                {
					...
					GameManager.instance.uIManager.vehicleSpeed = inputs.KMH.ToString("0"); //bhargav Code
				}
				else
