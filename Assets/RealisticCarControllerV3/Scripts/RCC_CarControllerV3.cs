...
public class RCC_CarControllerV3 : RCC_Core
{
    ...
    void Update()
    {
        ...
        //Reversing Bool.
        //if (!externalController){
        if (throttleInput >= 0.4f && !fuelSystem.IsVehicleMoving)
        {
            fuelSystem.IsVehicleMoving = true;
        }
        else
        {
            fuelSystem.IsVehicleMoving = false;
        }
    ...
    private void Engine()
    {
        //Speed. 
        //ashish Code
        if (inputs.throttleInput == 0 && inputs.brakeInput == 0)
        {
            myvalue = Mathf.InverseLerp(0, maxSpeed, rigid.velocity.magnitude) * 100f;
            if (speed < 5f)
            {
                myvalue = 1f;
            }
            else if (speed < 20f)
            {
                ...
                myvalue *= 4.2f;
            rigid.velocity = Vector3.Lerp(rigid.velocity, Vector3.zero, speed / myvalue * Time.fixedDeltaTime);
        }
        //ashish Code
        speed = rigid.velocity.magnitude * 1.2f; //3.6f
    ...
    private void Wheels()
    {
        ...
        for (int i = 0; i < allWheelColliders.Length; i++)
        {
            ...
            if (!appliedBrake && handbrakeInput > .1f) //.5f //Ashish Code
            {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.collider.tag == StringConstants.AI_Pedestrian)
        {
